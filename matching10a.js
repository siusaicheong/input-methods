function buildDatabases(corpus, n){
  ...
  for(var i=0;i<corpus.length;i++){
    ...
    var temp = [];
    if(current.length == 1){
      if (databaseGrams[current] === undefined){
          databaseGrams[current] = [i];
        } else {
          Array.prototype.push.apply(databaseGrams[current],[i]);
        }
      Array.prototype.push.apply(temp,[current]);
    } else {
...
function getNGrams(input, n){
  ...
  var output = [];
  if(current.length == 1){
    Array.prototype.push.apply(output, [current]);
  } else {
